...
if($Cache:AccessCondition -eq $true){
    ...
    #region PART PAGES VARIABLES
        # DEFAULT HOME PAGE
        $Cache:DefaultHomePageAccessAdministrator = '.'+'"'+"$Cache:DefaultDashboardsPath"+"$DashboardName"+'\pages\administrator.ps1'+'"'
        # DEFAULT HOME PAGE
        $Cache:DefaultHomePageAccessCustomer      = '.'+'"'+"$Cache:DefaultDashboardsPath"+"$DashboardName"+'\pages\customer.ps1'+'"'
        # DEFAULT HOME PAGE
        $Cache:DefaultHomePageAccessVisitor       = '.'+'"'+"$Cache:DefaultDashboardsPath"+"$DashboardName"+'\pages\visitor.ps1'+'"'
...
'@
                                )} -replace 'importcndata',$IndexNumber -replace 'importdomaindata',$Cache:DefaultCookiesGlobalOptions.Domain -replace 'importpathdata',$Cache:DefaultCookiesGlobalOptions.Path -replace 'importexpiredate',$Cache:DefaultCookiesGlobalOptions.Expire -replace 'importsecuredata',$Cache:DefaultCookiesGlobalOptions.Secure -replace 'importsamesitedata',$Cache:DefaultCookiesGlobalOptions.SameSite
                                $sc|iex
                        ...
                        if($ctGet -eq 'vc'){
                            ...
                            if(($true -eq $($CookieData.Id -eq $ciGet)) -and ($true -eq $($CookieData.Created -eq $cdGet))){
                                if(($true -ne ($CookieData.Id -eq $ssidData.Id)) -or ($true -ne ($CookieData.Cred -eq $ssidData.Cred)) -or ($true -ne ($CookieData.Lock -eq $ssidData.Lock)) -or ($true -ne ($CookieData.Vr -eq $ssidData.Vr)) -or ($true -ne ($CookieData.Uid -eq $ssidData.Uid)) -or ($true -ne ($CookieData.Pwd -eq $ssidData.Pwd)) -or ($true -ne ($CookieData.Expire -eq $ssidData.Expire)) -or ($true -ne ($CookieData.Created -eq $ssidData.Created))){
                                    ...
                                    $ExpireDateUtc = ($Date.AddDays($Expire)).ToUniversalTime()
...
'@
            ...
            New-UDHtml -Markup (
@'
    <div id="confirm-cookies-verification-container" style="display:block;position:fixed;left:0;bottom:0">
