        ...
        New-UDPaper -ClassName "mutual-ties-box-layout" -Square -Content {
            New-UDRow -Columns {
                New-UDColumn -SmallSize 12 -MediumSize 12 -LargeSize 12 {
                    ...
                    New-UDStyle -Content {
                        New-UDForm -Id 'Mutual-Ties-Overview-Mutual-Relations-Container-Form' -Children {
                                        ...
                                        New-UDTableColumn -Property Settings -Title ' ' -Width 0 -Render {
                                            ...
                                            New-UDElement -Id 'MutualTiesTableColumnCheckboxId' -Tag 'div' -Content {
                                                ...
                                                foreach ($CheckBoxItem in $Session:MutualTiesDataset) {
                                                    if(($($EventData.BindId) -eq $CheckBoxItem.Id) -and (($($EventData.ProviderName) -replace ' ') -eq $CheckBoxItem.Provider)){
                                                        Show-UDToast -Message $($EventData.BindId) -Duration 50000
                            ...
                            if($null -eq $MutualTiesList -or '' -eq $MutualTiesList){
                                Show-UDModal -Header {
                                    New-UDTypography -Text 'Really leave?' -Variant subtitle1
                                } -Content {
                                    New-UDButton -Text "Yes" -OnClick {
                                        $CreateFilePath          = "$Cache:MutualTieskDatabaseDatasetPath"+"$MutualTiesId"+'\'+"$MutualTiesProvider"+'.json'
                                        if($MutualTiesProvider){
                                            if($MutualTiesId){
                                                if(Test-Path $CreateFilePath){
                                                    $Remove = Remove-Item $CreateFilePath -Force
                                                    Invoke-UDRedirect -Url ("$Cache:ServerUrlAdress"+'app/dev/interface')
                                                }
                                                else{
                                                    Show-UDToast -Message 'Fullname Path is not found'
                                                }
                                            }
                                            else{
                                                Show-UDToast -Message 'Mutual Ties Id is not found'
                                            }
                                        }
                                        else{
                                            Show-UDToast -Message 'Mutual Ties Provider is not found'
                                        }
